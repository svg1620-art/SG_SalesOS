...
def upload():
    ...
    active_checklist = Checklist.query.filter_by(is_active=True).first()

    if request.method == "POST":
        if active_checklist is None:
            flash("Нет активного чек-листа — сначала активируйте чек-лист.", "error")
            return render_template(
                "calls/upload.html", managers=managers,
        ...
        # фиксируем, каким чек-листом будем оценивать, и запускаем пайплайн
        call.checklist_id = active_checklist.id
        db.session.commit()
...
def detail(call_id):
    ...
    return render_template("calls/detail.html", call=call, in_progress=IN_PROGRESS)
...
def panel(call_id):
    ...
    call = _get_call_or_404(call_id)
    return render_template("calls/_panel.html", call=call, in_progress=IN_PROGRESS)
