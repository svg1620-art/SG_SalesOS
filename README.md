...
>
> **Этапы 5–6 (текущие) — Диалоги + Дашборд РОПа.** Агрегация звонков в диалоги по
> нормализованному телефону (кол-во, средний балл, последняя зона, тренд); экран
> диалогов. Дашборд РОПа: KPI-карточки, donut распределения зон, лидерборд
> менеджеров с трендом, лента звонков с фильтрами (зона/менеджер/дата), красная
> зона подсвечена. Кабинет менеджера и amoCRM — следующие этапы.
