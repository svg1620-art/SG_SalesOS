...
>
> **Отделы.** Сущность отдела у пользователей; управление отделами; вкладки/фильтр
> по отделу на дашборде; Telegram-пульс по отделам; отдел в выгрузке CSV.
>
> **Этап 8 (текущее) — amoCRM.** Импорт звонков из amoCRM по долгосрочному токену:
> опрос примечаний call_in/call_out каждые `POLL_INTERVAL_MIN`, дедуп по
> `amo_note_id`, привязка менеджера (`User.amo_user_id` ↔ `responsible_user_id`) и
> клиента (по телефону), скачивание записи в Volume и запуск того же пайплайна.
> Настройка домена/токена и кнопки «Проверить подключение»/«Опросить сейчас» — в
> разделе «Настройки».
