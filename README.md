...
>
> **Этап 3 (текущий) — Ручная загрузка + пайплайн.** Загрузка аудио с привязкой
> менеджера/клиента (дедуп по SHA256), фоновый пайплайн: транскрибация со
> стерео-диаризацией (OpenAI) → анализ Claude по активному чек-листу → баллы,
> зона, саммери, рекомендации, упущенные моменты. Статус звонка обновляется
> polling'ом (HTMX). Карточка звонка с плеером и скоркартой (радар/подсветку
> добьём на Этапе 4). Дашборд-данные и amoCRM — следующие этапы.
