...
>
> **Этап 9 + выгрузки (текущее).** Дневная AI-сводка РОПа (Claude, cron) блоком
> на дашборде; экспорт транскрибаций по фильтрам (CSV/TXT) и всех диалогов (CSV);
> Telegram-пульс в 19:00 — по каждому менеджеру: диалогов, средняя длительность,
> балл, строка-рекомендация. amoCRM (Этап 8) — когда будут доступы.
